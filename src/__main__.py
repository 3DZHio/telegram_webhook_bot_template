...
async def on_startup(bot: Bot) -> None:
	...
	await bot.set_webhook(url=settings.webhook_url, drop_pending_updates=True if settings.DROP == 1 else None,
...
def main() -> None:
	...
	webhook_requests_handler = SimpleRequestHandler(dispatcher=dp, bot=bot,
	                                                secret_token=settings.WEBHOOK_SECRET.get_secret_value())
	webhook_requests_handler.register(app, path=f"/{settings.WEBHOOK_PATH.get_secret_value()}")
